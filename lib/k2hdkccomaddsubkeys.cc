...
//---------------------------------------------------------
K2hdkcComAddSubkeys::K2hdkcComAddSubkeys(K2HShm* pk2hash, ChmCntrl* pchmcntrl, uint64_t comnum, bool without_self, bool is_routing_on_server, bool is_wait_on_server) : K2hdkcCommand(pk2hash, pchmcntrl, comnum, without_self, is_routing_on_server, is_wait_on_server, DKC_COM_ADD_SUBKEYS)
{
	strClassName = "K2hdkcComAddSubkeys";
}

K2hdkcComAddSubkeys::~K2hdkcComAddSubkeys(void)
{
}
...
// Methods
//---------------------------------------------------------
PDKCCOM_ALL K2hdkcComAddSubkeys::MakeResponseOnlyHeadData(dkcres_type_t subcode, dkcres_type_t errcode)
{
	PDKCCOM_ALL	pComErr;
	if(NULL == (pComErr = reinterpret_cast<PDKCCOM_ALL>(malloc(sizeof(DKCRES_ADD_SUBKEYS))))){
		ERR_DKCPRN("Could not allocate memory.");
		return NULL;
	}
	PDKCRES_ADD_SUBKEYS	pComRes	= CVT_DKCRES_ADD_SUBKEYS(pComErr);
	pComRes->head.comtype		= DKC_COM_ADD_SUBKEYS;
	...
	pComRes->head.length		= sizeof(DKCRES_ADD_SUBKEYS);

	return pComErr;
}

bool K2hdkcComAddSubkeys::SetSucceedResponseData(void)
{
...
bool K2hdkcComAddSubkeys::CommandProcessing(void)
{
	...
	if(DKC_NORESTYPE == pRcvComAll->com_head.restype){
		...
		// get command data
		const PDKCCOM_ADD_SUBKEYS	pCom	= CVT_DKCCOM_ADD_SUBKEYS(pRcvComAll);
		...
		if(!pKey || !pSubkey){
			ERR_DKCPRN("Could not get safe key(%zd offset, %zu byte) and subkey(%zd offset, %zu byte) in DKCCOM_ADD_SUBKEYS(%p).", pCom->key_offset, pCom->key_length, pCom->subkey_offset, pCom->subkey_length, pRcvComAll);
		...
		if(NULL == (pSubKeys = pK2hObj->GetSubKeys(pKey, pCom->key_length, pCom->check_attr))){
			MSG_DKCPRN("Error or No subkeys for key(%s), but continue to adding new subkey.", bin_to_string(pKey, pCom->key_length).c_str());
			pSubKeys = new K2HSubKeys();
		}

		// insert subkey
		if(pSubKeys->end() == pSubKeys->insert(pSubkey, pCom->subkey_length)){
			ERR_DKCPRN("Failed to add subkey(%s) to key(%s) in DKCCOM_ADD_SUBKEYS(%p).", bin_to_string(pSubkey, pCom->subkey_length).c_str(), bin_to_string(pKey, pCom->key_length).c_str(), pRcvComAll);
		...
		if(!pSubKeys->Serialize(&pNewSubkeys, new_subkeys_length)){
			ERR_DKCPRN("Failed to serialize key(%s) after adding subkey(%s) in DKCCOM_ADD_SUBKEYS(%p).", bin_to_string(pKey, pCom->key_length).c_str(), bin_to_string(pSubkey, pCom->subkey_length).c_str(), pRcvComAll);
			SetErrorResponseData(DKC_RES_SUBCODE_INTERNAL);
			DKC_DELETE(pSubKeys);
			DKC_DELETE(pLock);
			return false;
		...
		if(!pK2hObj->ReplaceSubkeys(pKey, pCom->key_length, pNewSubkeys, new_subkeys_length)){
			ERR_DKCPRN("Failed to add subkey(%s) to key(%s) in DKCCOM_ADD_SUBKEYS(%p).", bin_to_string(pKey, pCom->key_length).c_str(), bin_to_string(pSubkey, pCom->subkey_length).c_str(), pRcvComAll);
			SetErrorResponseData(DKC_RES_SUBCODE_ADDSKEYLIST);
...
bool K2hdkcComAddSubkeys::CommandSend(const unsigned char* pkey, size_t keylength, const unsigned char* psubkey, size_t subkeylength, bool checkattr)
{
	...
	// make send data
	PDKCCOM_ALL	pComAll;
	if(NULL == (pComAll = reinterpret_cast<PDKCCOM_ALL>(malloc(sizeof(DKCCOM_ADD_SUBKEYS) + keylength + subkeylength)))){
		ERR_DKCPRN("Could not allocate memory.");
		return false;
	}
	PDKCCOM_ADD_SUBKEYS	pComAddSubkeys	= CVT_DKCCOM_ADD_SUBKEYS(pComAll);
	pComAddSubkeys->head.comtype		= DKC_COM_ADD_SUBKEYS;
	pComAddSubkeys->head.restype		= DKC_NORESTYPE;
	pComAddSubkeys->head.comnumber		= GetComNumber();
	pComAddSubkeys->head.dispcomnumber	= GetDispComNumber();
	pComAddSubkeys->head.length			= sizeof(DKCCOM_ADD_SUBKEYS) + keylength + subkeylength;
	pComAddSubkeys->key_offset			= sizeof(DKCCOM_ADD_SUBKEYS);
	pComAddSubkeys->key_length			= keylength;
	pComAddSubkeys->subkey_offset		= sizeof(DKCCOM_ADD_SUBKEYS) + keylength;
	pComAddSubkeys->subkey_length		= subkeylength;
	pComAddSubkeys->check_attr			= checkattr;

	unsigned char*	pdata				= reinterpret_cast<unsigned char*>(pComAddSubkeys) + pComAddSubkeys->key_offset;
	memcpy(pdata, pkey, keylength);
	pdata								= reinterpret_cast<unsigned char*>(pComAddSubkeys) + pComAddSubkeys->subkey_offset;
	memcpy(pdata, psubkey, subkeylength);

	if(!SetSendData(pComAll, K2hdkcCommand::MakeChmpxHash((reinterpret_cast<unsigned char*>(pComAddSubkeys) + pComAddSubkeys->key_offset), keylength))){
...
// Methods - Dump
//---------------------------------------------------------
void K2hdkcComAddSubkeys::RawDumpComAll(const PDKCCOM_ALL pComAll) const
{
	assert(DKC_COM_ADD_SUBKEYS == pComAll->com_head.comtype);

	if(DKC_NORESTYPE == pComAll->com_head.restype){
		PDKCCOM_ADD_SUBKEYS	pCom = CVT_DKCCOM_ADD_SUBKEYS(pComAll);

		RAW_DKCPRN("  DKCCOM_ADD_SUBKEYS = {");
		...
		//PDKCRES_ADD_SUBKEYS	pCom = CVT_DKCRES_ADD_SUBKEYS(pComAll);
		RAW_DKCPRN("  DKCRES_ADD_SUBKEYS = {");
